...
class AIServiceGenAI:
    ...
    def generate_itinerary(self, destination, duration, budget, themes):
        ...
        try:
            ...
            try:
                # Generate itinerary using Google GenAI with rate limiting
                ...
                if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg or "Quota exceeded" in error_msg:
                    print("⚠️  Quota limit reached - falling back to smart template system")
                    # Use the mock service for intelligent fallback
                    from services.ai_service_mock import AIServiceMock
                    mock_service = AIServiceMock()
                    return mock_service.generate_itinerary(destination, duration, budget, themes)
                else:
                    # Return basic itinerary with sample structure if AI fails
                    print("🔄 Falling back to basic template system")
                    ...
                    for day in range(1, duration + 1):
                        itinerary['daily_plans'].append({
                            ...
                            'activities': [
                                ...
                                {
                                    'time': '2:00 PM',
                                    'activity': f'Afternoon sightseeing in {destination}',
                                    'cost': daily_budget // 3,
                                    'duration': '4 hours'
                                },
                                {
                                    'time': '7:00 PM',
                                    'activity': f'Evening activities in {destination}',
                                    'cost': daily_budget // 3,
                                    'duration': '3 hours'
                                }
                            ]
                    ...
                    itinerary['data_sources'] = {
                        'influencer_recommendations': len(influencer_recommendations),
